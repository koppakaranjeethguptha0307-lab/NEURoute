...
API Request -> Backend Service (AIService) -> AI Adapter (AIIntegrationAdapter) -> Real AI Team Code (ai/services/*) -> Prediction Result -> Database Persistence -> API Response.
"""
...
@pytest.mark.asyncio
async def test_ai_adapter_uses_real_ai_and_not_mock():
    """Prove that AIIntegrationAdapter uses REAL AI team code and not MockAIAdapter."""
    adapter = AIIntegrationAdapter()

    # Assert adapter reports using real AI
    assert adapter.is_using_real_ai is True
    assert adapter.force_mock is False
    assert adapter.classifier is not None
    assert adapter.risk_predictor is not None
    assert adapter.delay_estimator is not None
    assert adapter.route_optimizer is not None

    # Test real classifier execution
    class_res = await adapter.classify_incident_text("Torrential rain causing mudslide on highway")
    assert class_res.is_fallback is False  # Proves real AI executed, not mock fallback!
    assert class_res.confidence > 0.80

    # Test real segment risk predictor execution
    risk_res = await adapter.predict_segment_risk(
        segment_id=101,
        rainfall_mm=110.0,
    ...
    assert risk_res.is_fallback is False  # Proves real AI executed, not mock fallback!
    assert risk_res.risk_score >= 0.60
    assert risk_res.risk_level in ["HIGH", "CRITICAL"]

    # Test real delay estimator execution
    delay_res = await adapter.estimate_delay(
        distance_km=100.0,
        base_speed_kmh=50.0,
        segment_risk_scores=[0.75],
        active_bottlenecks_count=1
    )
    assert delay_res.is_fallback is False  # Proves real AI executed, not mock fallback!
    assert delay_res.estimated_delay_hours > 0.0

    # Test real route optimizer execution
    opt_res = adapter.optimize_routes(
        candidate_routes=[
            {"id": "r1", "distance_km": 50, "risk_score": 0.1},
            {"id": "r2", "distance_km": 40, "risk_score": 0.9}
        ],
        cargo_priority="CRITICAL",
        preference="SAFEST"
    )
    assert opt_res["recommended_route_id"] == "r1"
    assert "CRITICAL cargo safety weighting" in opt_res["reason"]  # Rationale generated by real RouteOptimizer!


def test_ai_service_persistence(db_session: Session):
    """Verify AIService computes predictions via AIIntegrationAdapter and persists them in DB."""
